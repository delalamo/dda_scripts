...
def _args() -> Tuple[List[str], str, List[int], str, str]:
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "(optional) Reference PDB for structural alignment. "
    ...
    if "ref" not in args:
        ...
        logging.warning(
            "No reference PDB provided! Using model {}.".format(args["ref"])
        )

    if args["ignore"] is not None:
        if args["keep"] is not None:
            logging.warning(
                "Both --res_to_keep and --res_to_ignore options "
...
def read_pdb(
    pdbfile: str, res_list: List[int] = [], atoms: List[str] = ["CA"]
) -> Dict[int, np.ndarray]:
    ...
    if not isempty and len(residues_left) > 0:
        logging.warning(
            "\n".join(
                (
                    "Some residues not read or absent from PDB!",
...
def _pca(all_xyz: np.ndarray, model_names: List) -> PCA:
...
def calc_tmscore(tmalign: str, pdb1: str, pdb2: str) -> float:
...
def _main(outpath: str = "temp") -> NoReturn:
    ...
    for model in args["models"]:
        ...
        if tm > args["cutoff"]:
            model_tms[name] = tm
            cmd = " ".join((args["tmalign"], model, args["ref"], "-o", outname))
